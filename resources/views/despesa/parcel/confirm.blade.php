<x-app-layout>
    ...
    <x-slot name="title">
        Confirmar parcelamento de despesa
    </x-slot>

    <div class="ui container">

        <div class="ui info message">
            <p>Confirme as informações para o parcelamento da despesa:</p>
        </div>


        <div class="ui segment">
            <x-form title="Repetição de despesa" action="{{ route('despesa.parcel.store') }}" method="POST">
                @foreach ($despesas as $despesa)
                    ...
                    <x-input.localizador value="{{ old('localizador', $despesa['localizador']) }}"
                        name="localizador[{{ $despesa['parcela'] }}]" id="localizador[{{ $despesa['parcela'] }}]" />
                @endforeach
            </x-form>
        </div>
