<x-app-layout>
    ...
    <x-slot name="title">
        Confirmar parcelamento da receita
    </x-slot>

    <div class="ui container">

        <div class="ui info message">
            <p>Confirme as informações para o parcelamento da receita:</p>
        </div>


        <div class="ui segment">
            <x-form title="Parcelamento de receitas" action="{{ route('receita.parcel.store') }}" method="POST">
                @foreach ($receitas as $receita)
                    ...
                    <x-input.devedor value="{{ old('devedor', $receita['devedor']) }}" :devedores="pessoas()" required name="devedor[{{ $receita['parcela'] }}]" id="devedor[{{ $receita['parcela'] }}]" />
